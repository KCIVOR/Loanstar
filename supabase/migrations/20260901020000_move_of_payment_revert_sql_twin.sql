-- Move of Payment — Phase 3 SQL twin (see
-- docs/revision-plans/feature-move-of-payment-implementation-plan.md).
--
-- The TypeScript refreshMasterlistAging (src/lib/ar/posting.ts) is NOT the
-- only caller of this logic — a real, active nightly pg_cron job
-- ("loanstar-aging-daily", supabase/migrations/20260717102046_aging_refresh_cron.sql)
-- calls refresh_all_aging() -> refresh_one_masterlist_aging() entirely in
-- SQL, for every active/remedial account, once a day, completely bypassing
-- the TS function. Without this migration, a moved installment would only
-- ever revert if a human happened to trigger the TS path for that specific
-- account (dev-simulate-aging, or a Collector accounts-list GET) — the
-- primary automatic mechanism would silently miss it. This was found by
-- checking for a SQL-side caller before considering Phase 3 done, not
-- assumed to only live in TypeScript.
--
-- Only the new revert block is added, in the same position as the TS
-- twin (before the overdue-detection SELECT, so that untouched query
-- naturally sees any just-reverted row's fresh status on this same pass).
-- Every other line of this function is unchanged from its current live
-- definition (re-fetched via pg_get_functiondef immediately before writing
-- this migration, not assumed from an older migration file).
...
BEGIN
  ...
  -- Move of Payment revert (Phase 3) — standalone block, run before the
  -- overdue-detection SELECT below. Every row sharing a
  -- move_of_payment_batch_id always carries the same
  -- move_of_payment_deadline (set together in applyMoveOfPayment), so this
  -- single UPDATE...WHERE reverts a whole batch at once, never half of it.
