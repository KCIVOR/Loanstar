-- Principal-first payments (Phase 5 of principal-first-fee-stays-plan.md).
--
-- recompute_account_penalties (Phase 1) now reads a row whose PRINCIPAL is fully
-- covered but whose fee is still owed as 'partial', not 'overdue'. The nightly
-- refresh_one_masterlist_aging still hard-set every row it charged to 'overdue',
-- so the cron would flip a legitimately principal-paid month back to 'overdue'
-- the next night. Make the two engines agree.
--
-- Surgical scope: ONE line — the accrual loop's UPDATE `status = 'overdue'`
-- becomes a CASE that yields 'partial' when the principal is covered. Every
-- other block (move-of-payment reverts, aging ladder, discount reversion, the
-- masterlist UPDATE, the compounding base with its fee_paid carve-out) is
-- verbatim.
...
BEGIN
  ...
  -- Target (amount_due) is never changed; the installment stays independently
  -- payable. penalty_periods_applied makes same-day re-runs no-ops.
  IF v_has_overdue THEN
    FOR v_row IN
      SELECT
        s.id,
    ...
    LOOP
      ...
      SET
        ...
        -- Principal covered, fee still owed -> 'partial'; matches
        -- recompute_account_penalties (2026-09-10). Otherwise 'overdue'.
        status = CASE
                   WHEN GREATEST(0, v_row.amount_paid - v_row.fee_paid)
                        >= GREATEST(0, v_row.amount_due - v_row.discount_amount) - 0.005
                   THEN 'partial'
                   ELSE 'overdue'
                 END
      WHERE id = v_row.id;
