-- Phase 5 of the Collector Discount in DCRR feature (continued). See
-- docs/revision-plans/feature-collector-discount-implementation-plan.md.
--
-- The "amount_due - discount_amount + penalty_amount - amount_paid" formula
-- lives in exactly four places in this codebase, confirmed by reading each
-- directly — post_single_dcr_item (fixed in the prior migration),
-- recompute_outstanding_balance, netInstallmentDue (TS, already fixed), and
-- the 30-day rollover in both aging twins. All four must change together or
-- the money will disagree with itself — this codebase has already been
-- bitten once by fixing only some copies of this exact formula (see
-- docs/payment-flow-discount-audit-and-fix-plan.md).

-- 1. recompute_outstanding_balance — account-wide balance, independent of
-- the per-item posting fixed above. Only additive change: subtract
-- penalty_discount_amount alongside the existing discount_amount term.
...
-- 2. refresh_one_masterlist_aging — the 30-day rollover folds an overdue
-- row's unpaid balance PLUS its penalty into the next installment's
-- amount_due. Without this fix, a Collector-waived penalty would roll
-- forward at full (gross) value, silently resurrecting the waived amount
-- as debt on a different row. Two changes: (a) the v_overdue fetch now
-- also selects penalty_discount_amount — a missing column in one of two
-- duplicated copies is precisely the failure mode this codebase has
-- already been bitten by once (the discount_amount audit, 2026-08-31);
-- (b) the roll-amount formula subtracts it. Also: once a row is marked
-- 'rolled', its penalty_discount_amount is zeroed — the waiver has been
-- consumed by the roll calculation, so a stale figure left behind would
-- invite double-counting if that row is ever re-read.
--
-- Everything else in this function (move-of-payment cleanup, penalty
-- accrual, origination-discount reversion) is unchanged — confirmed by
-- diffing against the live definition read directly before this edit.
...
BEGIN
  ...
  IF v_has_overdue AND v_dpd >= 1 THEN
    ...
    IF v_dpd >= v_t30 AND v_overdue.rolled_at IS NULL THEN
      ...
      IF v_has_next THEN
        -- Net of both the interest-side discount AND the Collector
        ...
        SET
          ...
          -- Consumed by the roll calculation above — zero it so a stale
          -- figure on a rolled row can't be double-counted later.
