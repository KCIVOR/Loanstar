-- SQL twin of the TS fix in src/lib/ar/posting.ts (30-day rollover block).
-- See docs/quarterly-bimonthly-special-schedule-implementation-plan.md and
-- feature-collector-discount-implementation-plan.md.
--
-- Bug (confirmed live 2026-09-04, AN300449): the 30-day rollover folded an
-- overdue installment's whole net-due (interest + accrued penalty) into the
-- destination installment's `amount_due` as one undifferentiated number.
-- That destination row's own `penalty_amount` column never picked up the
-- rolled-forward penalty, so the Collector Discount UI's "penalty discount"
-- picker (which requires penaltyAmount > 0 per installment) had nothing to
-- show — a real, rolled-forward penalty became permanently un-discountable.
--
-- Fix: split the roll amount into its interest and penalty portions and
-- write each into its own column on the destination row. The two portions
-- are computed so they always sum to exactly the same v_roll_amount as
-- before — the destination row's total net-due is unchanged, only which
-- column carries which part of it. Everything else in this function
-- (move-of-payment cleanup, penalty accrual, origination-discount
-- reversion, aging bucket, masterlist totals) is unchanged — confirmed by
  ...
  IF v_has_overdue AND v_dpd >= 1 THEN
    ...
    IF v_dpd >= v_t30 AND v_overdue.rolled_at IS NULL THEN
      ...
      IF v_has_next THEN
        ...
        -- Split into interest/penalty portions instead of dumping the whole
        -- amount into amount_due — see migration header comment. The two
        -- portions always sum to exactly v_roll_amount.
