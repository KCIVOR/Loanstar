-- Move of Payment — Addendum 2, Phase 3 SQL twin (see
...
-- only caller of the revert logic — the nightly pg_cron job
-- "loanstar-aging-daily" runs refresh_all_aging() -> refresh_one_masterlist_aging()
-- entirely in SQL, bypassing the TS path. That function already reverts a
-- lapsed moved batch's rows (status 'moved' -> 'pending', added in
-- 20260901020000). This migration adds the OTHER half of Addendum 2 Gap 1:
-- when a batch lapses, also delete the installment row(s) that same batch
-- appended to the end of the schedule (tagged with
-- deferred_from_move_of_payment_batch_id — a separate column from
-- move_of_payment_batch_id, see 20260901030000 and the plan's validation
-- pass for why). Rows that already carry a payment are left alone.
--
-- The whole function body below is the CURRENT LIVE definition
-- (re-fetched via pg_get_functiondef immediately before writing this
-- migration, not copied from an older migration file), with exactly one
-- new statement added: the DELETE block, placed immediately BEFORE the
-- existing status revert UPDATE so the source rows are still status =
-- 'moved' and still carry their move_of_payment_batch_id when the subquery
-- reads them. Every other line is unchanged.
...
BEGIN
  ...
  -- Move of Payment extension revert (Addendum 2, Gap 1) — NEW. Runs before
  -- the status revert UPDATE below so the lapsed batches are still
  -- identifiable by move_of_payment_batch_id. Never deletes a row that
  -- already carries a payment.
