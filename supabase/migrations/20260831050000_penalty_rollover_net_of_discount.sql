-- Payment-flow discount audit (docs/payment-flow-discount-audit-and-fix-plan.md,
-- Phase 3): penalty and the 30-day rollover were both computed on the
-- overdue installment's GROSS amount_due, never subtracting an active
-- discount. In normal day-by-day operation this is usually masked — the
-- discount-reversion step below already clears discount_amount to 0 one day
-- before penalty ever applies (dpd >= 0 vs dpd >= 1) — but any time both run
-- in the same pass (a cron gap, or a large "Simulate delinquency" jump,
-- confirmed live today), a still-discounted overdue installment is
-- penalized and rolled forward on more than it actually owes. Mirrors the
-- identical fix already applied to refreshMasterlistAging() in
...
BEGIN
  ...
  IF v_has_overdue AND v_dpd >= 1 THEN
    ...
    -- Net of discount and already-paid — what's really still owed.
    ...
    IF v_dpd >= v_t30 AND v_overdue.rolled_at IS NULL THEN
      ...
      IF v_has_next THEN
        -- Net of discount — roll forward what's really still owed, not the
        -- gross amount_due.
