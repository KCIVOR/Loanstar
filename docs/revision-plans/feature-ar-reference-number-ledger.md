...
- [ ] Live: Collector attempts to record a payment with no reference number — blocked. Records one with a reference number — succeeds, visible on the ledger column and in payment history.
- [ ] Live: as the borrower, confirm the reference number appears on their own ledger view and payment history.
- [ ] Live: AR confirms/posts that payment — confirm Collector can no longer edit it, Super Admin still can.
