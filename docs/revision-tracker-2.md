...
- Item 8 — AR: Reference/transaction number on amortization ledger — [feature-ar-reference-number-ledger.md](revision-plans/feature-ar-reference-number-ledger.md), audited + planned 2026-08-14
- Item 9 — AR: Rounding write-off page — [feature-ar-rounding-writeoff-page.md](revision-plans/feature-ar-rounding-writeoff-page.md), audited + planned 2026-08-17
...
| # | Item | Status | Notes |
|---|------|--------|-------|
| 7 | Add a configurable coded BIR / Non-BIR status field to AR master list accounts (no literal "BIR"/"Non-BIR" labels in UI), editable by AR, filterable/sortable, visible in account detail, codes configurable by Super Admin, never exposed to the borrower portal | Done | Implemented by Cursor 2026-08-14, all 3 phases validated (code + `tsc` + full suite 903/903 checked directly, live DB/RLS state confirmed, borrower-exclusion fix independently verified field-by-field for regressions), merged to `main` — see `revision-plans/feature-ar-bir-status-tagging.md`. Zero literal "BIR"/"Non-BIR" text confirmed anywhere in the UI. Unknown/removed codes fall back to showing the raw code rather than crashing — verified directly in the diff. **Post-merge hotfix found live (2026-08-14, commit `59d0c0e`)**: the new `bir-status-codes` read route used the regular RLS-bound client to read `config_settings`, which is gated to `system_config` — AR doesn't have that module, so the codes never actually loaded. Fixed via `createServiceClient()` for that one narrow, already-permission-checked read. Reviewed and confirmed correct. |
| 8 | Add a required "Reference / Transaction No." free-text field to the payment posting form (collector + AR), surfaced as a ledger column and in the borrower portal's payment history; locked after AR confirms posting (correction requires Super Admin) | Done | Implemented by Cursor 2026-08-14, all 3 phases validated (code + full suite 903/903, live RLS confirmed). Require-at-entry on Collector/Remedial `RecordPaymentModal` + API; Reference No. column on borrower + AR amortization ledgers; `payments_collector_update` status-gated with Super Admin escape hatch. RLS deviation from plan literal: WITH CHECK is permission-only (not status-gated) so DCR submit (pending→confirmed) and AR reconcile (confirmed→posted) keep working; AR may still update confirmed rows only to post them, and cannot touch posted. Flag: borrower portal payment POST still has optional `referenceNo`. See `revision-plans/feature-ar-reference-number-ledger.md`. |
| 9 | Dedicated Rounding Write-Off page: a "Clear Rounding Difference" action on the account/ledger view (visible below a Super-Admin-configured threshold), logging borrower, account/loan no., amount, timestamp, and acting AR user; page filterable by date range/borrower/AR user with summarized totals; confirmation dialog; irreversible; AR + Super Admin only | Done | Implemented directly by Claude 2026-08-17 (user explicitly requested direct implementation, deviating from the plan-then-Cursor default) — both phases, code + `tsc` + full suite 1007/1007 checked directly, live walkthrough confirmed as both AR and a real Collector session (403 parity with `/api/ar/history/accounts` confirmed via SQL role check). See `revision-plans/feature-ar-rounding-writeoff-page.md`. New page `/ar/rounding-writeoffs`, new API route, new sidebar nav entry, small cross-link added from the masterlist detail page's existing "Rounding write-offs" card. Not yet committed — left for the user. |
