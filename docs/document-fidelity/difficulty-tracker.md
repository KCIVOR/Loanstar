...
| — | `demand_letter_sme` | lra | *(draft v1, unpublished — HTML)* | Fixed — tagged docx replacements prepared from two real filled samples for the same real client (PRODUCERS CONNECTION LOGISTICS INC., attention Arturo Jr. Esguerra Villanuevba/Jennifer Varon Villanueva, account no. 163-6-000089-55): `Demand Letter - SME.docx` (has-address variant) and `Demand Letter - SME No Address.docx` (no-address variant, structurally omits the address block rather than just blanking it). Reused every field already established by the unpublished v1 HTML draft and the sibling `demand_letter_dishonored_check` template — no new fields needed: `borrowerName`, `address`, `borrowerRepresentative`/`borrowerRepresentativeTitle` (wrapped in `{{#borrowerRepresentative}}…{{/borrowerRepresentative}}` spanning both table rows of the Attention block, mirroring the HTML's `data-if="borrowerRepresentative"`), `companyName`, `demandCheckAccountNo`, `demandReason`, and the `{{#demandChecks}}…{{/demandChecks}}` loop (`bankName`/`checkNumber`/`checkDate`/`amount`) reused from `demand_letter_dishonored_check`. The checks section in both source samples turned out to be tab-stop/space-padded header text with a genuinely blank data line (no real check ever filled in) rather than a real table — collapsed that blank line into the loop row. Several source values (address, both Attention fields, the account no. in the no-address file) were wrapped in Word `LINK`/`MERGEFORMAT` fields pointing at a nonexistent local Excel file; per the skill's known-gotcha guidance, every MERGEFORMAT-bearing field was stripped down to a plain tagged run, the few without MERGEFORMAT had just their `<w:t>` edited in place. The has-address file's address paragraph is wrapped in `{{#hasAddress}}{{address}}{{/hasAddress}}{{^hasAddress}}` + a literal non-breaking-space run `{{/hasAddress}}`, mirroring `demand_letter_v2`'s own `hasAddress`/`data-unless` convention; the no-address file has no address paragraph at all to gate, matching the source. Verified with a pizzip+docxtemplater merge test (matching `docx-merge.ts`'s config) against a 3-item `demandChecks` context, in `hasAddress: true`/`false` and `borrowerRepresentative` present/absent states on both files — zero unresolved tags, and a case-insensitive sweep of both tagged XML files confirmed zero leftover literal client values. No visual/PDF verification was possible in this environment (no Gotenberg egress, no working local LibreOffice) — flagged plainly, not claimed. Neither file has been uploaded to Storage or published; the current `demand_letter_sme` draft stays HTML/v1/unpublished, so there is no live-data risk (unlike the `blri`/voucher incidents) — this is new-template prep, not a fix to something already serving real borrowers |
