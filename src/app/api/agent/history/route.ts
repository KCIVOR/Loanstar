...
import {
  ...
  getClosedLeadsHistory,
  getClosedLeadsKpiCounts,
  type ClosedLeadsSortKey,
...
export async function GET(request: Request) {
  try {
    ...
    const sortKeyRaw = searchParams.get("sortKey") ?? "convertedAt";
    const sortKey = (
      SORT_KEYS.has(sortKeyRaw) ? sortKeyRaw : "convertedAt"
    ) as ClosedLeadsSortKey;
    const sortDirRaw = searchParams.get("sortDir") ?? "desc";
    const sortDir = sortDirRaw === "asc" ? "asc" : "desc";

    const page = Math.max(1, Number(searchParams.get("page") ?? 1) || 1);
    const pageSize = clampClosedLeadsPageSize(
      Number(searchParams.get("pageSize") ?? 10),
    );

    const supabase = await createClient();
    const [history, kpi] = await Promise.all([
      getClosedLeadsHistory(supabase, user.id, {
        search,
      ...
      getClosedLeadsKpiCounts(supabase, user.id, { from, to }),
    ]);
