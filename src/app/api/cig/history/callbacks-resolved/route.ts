...
import {
  ...
  getCigCallbacksResolvedHistory,
  getCigCallbacksResolvedKpiCounts,
...
const SORT_KEYS = new Set(["resolvedAt", "applicationNo", "scheduledAt"]);

/** Resolved-callback history. Read-only. */
export async function GET(request: Request) {
  try {
    ...
    const sortKeyRaw = searchParams.get("sortKey") ?? "resolvedAt";
    const sortKey = (
      SORT_KEYS.has(sortKeyRaw) ? sortKeyRaw : "resolvedAt"
    ) as CigCallbacksResolvedSortKey;
    ...
    const [history, kpi] = await Promise.all([
      getCigCallbacksResolvedHistory(supabase, {
        search,
        from,
        to,
        sortKey,
      ...
      getCigCallbacksResolvedKpiCounts(supabase, { from, to }),
    ]);
