...
import {
  ...
  getCigReturnedHistory,
  getCigReturnedKpiCounts,
...
const SORT_KEYS = new Set(["returnedAt", "applicationNo", "borrower"]);
const SEGMENT_FILTERS = new Set(["all", "seafarer", "sme", "individual"]);

/** Returned-to-CSA history. Read-only. */
export async function GET(request: Request) {
  try {
    ...
    const sortKeyRaw = searchParams.get("sortKey") ?? "returnedAt";
    const sortKey = (
      SORT_KEYS.has(sortKeyRaw) ? sortKeyRaw : "returnedAt"
    ) as CigReturnedSortKey;
    ...
    const [history, kpi] = await Promise.all([
      getCigReturnedHistory(supabase, {
        search,
        segment,
        from,
        to,
      ...
      getCigReturnedKpiCounts(supabase, { from, to }),
    ]);
