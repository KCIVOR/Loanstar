...
import {
  ...
  getCigDenialCallsHistory,
  getCigDenialCallsKpiCounts,
...
const SORT_KEYS = new Set(["informedAt", "applicationNo", "borrower"]);

/** Completed denial-call history. Read-only. */
export async function GET(request: Request) {
  try {
    ...
    const sortKeyRaw = searchParams.get("sortKey") ?? "informedAt";
    const sortKey = (
      SORT_KEYS.has(sortKeyRaw) ? sortKeyRaw : "informedAt"
    ) as CigDenialCallsSortKey;
    ...
    const [history, kpi] = await Promise.all([
      getCigDenialCallsHistory(supabase, {
        search,
        from,
        to,
        sortKey,
      ...
      getCigDenialCallsKpiCounts(supabase, { from, to }),
    ]);
