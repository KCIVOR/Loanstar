...
import {
  ...
  getCigCancellationsHistory,
  getCigCancellationsKpiCounts,
...
const SORT_KEYS = new Set(["cancelledAt", "applicationNo", "borrower"]);

/** Cancelled-application history. Read-only. */
export async function GET(request: Request) {
  try {
    ...
    const sortKeyRaw = searchParams.get("sortKey") ?? "cancelledAt";
    const sortKey = (
      SORT_KEYS.has(sortKeyRaw) ? sortKeyRaw : "cancelledAt"
    ) as CigCancellationsSortKey;
    ...
    const [history, kpi] = await Promise.all([
      getCigCancellationsHistory(supabase, {
        search,
        from,
        to,
        sortKey,
      ...
      getCigCancellationsKpiCounts(supabase, { from, to }),
    ]);
