...
/** Common seafarer / OFW destinations first, then alphabetical. */
const PRIORITY_ISO = [
  "PH",
  "SG",
  "HK",
...
export const DEFAULT_DIAL_COUNTRY =
  byIso.get("PH") ?? ALL.find((c) => c.iso2 === "PH")!;

function priorityRank(iso2: string): number {
  const idx = PRIORITY_ISO.indexOf(iso2 as (typeof PRIORITY_ISO)[number]);
  return idx === -1 ? PRIORITY_ISO.length + 1 : idx;
}

/** Priority destinations first, then A–Z by name. */
export const DIAL_COUNTRIES: DialCountry[] = [...ALL].sort((a, b) => {
  const ra = priorityRank(a.iso2);
  const rb = priorityRank(b.iso2);
  if (ra !== rb) return ra - rb;
  return a.name.localeCompare(b.name);
});

export function findDialCountry(iso2: string): DialCountry | undefined {
  return byIso.get(iso2);
}

export function filterDialCountries(query: string): DialCountry[] {
  ...
  return DIAL_COUNTRIES.filter((c) => {
    ...
    if (c.dial.includes(q) || (digits && c.dial.replace("+", "").startsWith(digits))) {
      return true;
    }
    return false;
  });
...
/** Strip non-digits and a leading trunk 0 (e.g. PH 0917… → 917…). */
export function normalizeNationalNumber(raw: string): string {
  const digits = raw.replace(/\D/g, "");
  return digits.replace(/^0+/, "");
}

export function formatE164(dial: string, national: string): string | undefined {
  const n = normalizeNationalNumber(national);
  if (!n) return undefined;
  return `${dial}${n}`;
}

const DIAL_LENGTHS = Array.from(new Set(ALL.map((c) => c.dial.length - 1))).sort(
  (a, b) => b - a,
);

/**
 * Split a stored phone value into a country + national number for the
 * picker. Handles proper E.164 ("+639171234567") via longest-dial-prefix
 * match, and falls back to the default country for legacy values saved
 * before the country picker existed (e.g. plain "09171234567").
 */
export function parseE164(raw: string | null | undefined): {
  ...
  if (!trimmed) return { country: DEFAULT_DIAL_COUNTRY, national: "" };

  if (trimmed.startsWith("+")) {
    const digits = trimmed.slice(1).replace(/\D/g, "");
    for (const len of DIAL_LENGTHS) {
      const prefix = `+${digits.slice(0, len)}`;
      const match = ALL.find((c) => c.dial === prefix);
      if (match) return { country: match, national: digits.slice(len) };
    }
    return { country: DEFAULT_DIAL_COUNTRY, national: digits };
  }

  return { country: DEFAULT_DIAL_COUNTRY, national: normalizeNationalNumber(trimmed) };
}
