/**
 * `loan_agreement_vienovo` — fidelity fix against the most recently-dated
 * Vienovo source, `Vienovo Loan Agreement - quarter2months 02.20.26.doc`
 * (cross-checked against the earlier-dated `... 02.05.26.doc` and the
 * undated `... quarter2months.doc` blank template — all 3 agree on wording,
 * differing only in the filled-in loan figures/dates, so there is no
 * discrepancy between them to flag).
 *
 * The published body already matched the source closely (recipient block,
 * `isCorporateBorrower` conditional, `isQuarterly`/`isEvery2Months`
 * conditional for Clause 3's "quarterly"/"an every-two-months" basis,
 * notarial block) — confirmed by rendering it through the real Gotenberg
 * pipeline and comparing page geometry against the source.
 *
 * One real content gap found and fixed here: all 3 Vienovo sources' Clause
 * 3 has a second paragraph spelling out the per-check PDC amount —
 * "the Borrower shall issue to the Lender Four (4) Post-Dated Checks with
 * each check[s] is amounting to ONE HUNDRED FIFTY THOUSAND PESOS (Php
 * 150,000.00) from April 28, 2026 until August 28, 2026, and FIVE MILLION
 * PESOS (Php 5,000,000.00) on August 28, 2026" — but the published body's
 * equivalent paragraph dropped the amount/date detail entirely ("...Post-
 * Dated Checks in favor of the Lender, to be deposited as dated..."). The
 * sibling `loan_agreement` slug's own Clause 3 (isStandardSchedule branch)
 * already carries the equivalent sentence — "...Post-Dated Checks with
 * each check amounting to {{perCheckAmountInWords}} (Php {{perCheckAmount}})
 * from {{loanStartDate}} until {{loanMaturityDate}}." — using merge keys
 * that already exist in template-context.ts. This fix reuses that exact,
 * already-reviewed pattern (no new merge fields needed).
 *
 * Not fixed: the source's own final-check clause ("...and FIVE MILLION
 * PESOS on August 28, 2026") describes a distinct balloon payment (the
 * final PDC equals the full principal, not `perCheckAmount`) that the
 * current PDC data model has no clean single merge key for — reusing
 * `{{perCheckAmount}}` for every check (as done here, matching the sibling
 * `loan_agreement` template's own simplification for its own balloon-free
 * variants) is a safe, house-consistent improvement over the prior blank
 * sentence, but does not literally reproduce the balloon-amount clause.
 * Flagged for a follow-up merge-field addition rather than guessed here.
 *
 * Built only from primitives declared in extensions.ts (this body predates
 * the `data-align` convention and still uses the tolerated legacy
 * `style="text-align:center"` form on the two headings, which
 * `mergeTemplate`/the editor's `parseAlign` normalise on load — see
 * extensions.ts's `AlignAttribute` comment — so it is left as-is rather
 * than churned to `data-align` for no behavior change).
 */
export const SOURCE_FAITHFUL_LOAN_AGREEMENT_VIENOVO_PREVIEW = String.raw`
